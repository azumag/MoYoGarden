(() => {
  ...
  const stableFallback = (reason) => {
    ...
    setMessage("高解像度版を起動できませんでした", "安定版へ切り替えています");

    const stableHost = new URL(STABLE_URL).host;
    if (location.host !== stableHost) {
      const destination = new URL(STABLE_URL);
    ...
    if (detail) {
      ...
      link.href = STABLE_URL;
      link.textContent = "安定版を開く";
