...
test("header-scoped canonical meta stays sparse without REGION_IDS enumeration", async () => {
  let regionListReads = 0;
  const env = {
    DEFAULT_REGION_ID: "garden-1",
    get REGION_IDS() {
      regionListReads += 1;
      throw new Error("canonical scoped meta must not enumerate REGION_IDS");
    },
  };

  const response = await workerEntry.fetch(
    new Request("https://moyo.test/api/meta?radius=1", {
      headers: { "x-moyo-region": "hex-q2-r-1" },
    }),
    env,
  );
