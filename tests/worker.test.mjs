...
test("region window follows logical hex distance instead of array index distance",()=>{const ids=Array.from({length:19},(_,index)=>`garden-${index+1}`);assert.deepEqual(regionWindow(ids,"garden-1",1,40,24).map((entry)=>entry.id),["garden-1","garden-2","garden-3","garden-4","garden-5","garden-6","garden-7"]);assert.deepEqual(regionWindow(ids,"garden-3",1,40,24).map((entry)=>entry.id),["garden-1","garden-2","garden-3","garden-4","garden-9","garden-11","garden-12"]);assert.deepEqual(regionWindow(ids,"garden-3",0,40,24).map((entry)=>entry.id),["garden-3"]);assert.deepEqual(regionWindow(ids,"garden-1",2,40,24).map((entry)=>entry.id),ids);assert.deepEqual(regionWindow(ids,"missing",1,40,24),[]);});
